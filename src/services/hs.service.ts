...
export class HsService {
  ...
  async upsert(items: Array<ContactAccountPartial>) {
    ...
    for (let item of items) {
      if (item.Email) {
        if (new RegExp(/^[\w.+\-]+@[\w\-]+\.[a-z]{2,}$/i).test(item.Email)) {
          ...
          this.logger.error(
            `Email ${item.Email} is invalid, for item: ${JSON.stringify(item)}`,
          );
        }
      } else {
    ...
    if (update.length) {
      await this.batchUpdateCompaniesAndContacts(update);
    }
    if (create.length) {
  ...
  private async createCompanyWithAssociatedContact({
    ...
    try {
      ...
      const associationTypes =
        await this.client.crm.associations.typesApi.getAll(
          `company`,
          `contacts`,
        );
  ...
  private async batchUpdateCompaniesAndContacts(
    update: Array<ContactAccountPartial>,
  ) {
    try {
      ...
      const companyBatch: CompanyUpdateBatch = {
        inputs: batch.map((i) => {
          ...
          this.logger.debug(
            `batchUpdateCompaniesAndContacts: company : hs_object_id: ${id} from sfId: ${i.company.salesforceaccountid}`,
          );
          return {
            id,
            properties: i.company,
          };
        }),
      };
      this.logger.info(
      ...
      const contactBatch: ContactUpdateBatch = {
        inputs: batch.map((i) => {
          ...
          this.logger.debug(
            `batchUpdateCompaniesAndContacts: contact : hs_object_id: ${id} from sfId: ${i.contact.salesforceaccountid}`,
          );
          return {
            id,
            properties: i.contact,
          };
        }),
      };
      this.logger.info(
  ...
  private mapContactAccountPartial(
    ...
    return items.map((item: ContactAccountPartial) => {
      ...
      const company = {
        ...
        zip: item.Account.ShippingAddress?.postalCode,
      };
      const contact = {
        email: item.Email,
        phone: item.Phone,
